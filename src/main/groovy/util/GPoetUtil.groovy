...
//            println "p.type====$p.type"
    ...
    //创建之前将packageName修改成对应的模块名
    def tempParamPackageName = "$Config.PACKAGE_NAME.REQUEST.$Config.CURRENT_GROUP_NAME"
    //创建
    print2File(Config.FILE_PATH.REQUEST, tempParamPackageName, paramType.build())
    //包名+文件名就是它的TypeName
    return ClassName.get(tempParamPackageName, paramName)
}
...
//生成className的方法
    ...
    def tempResponsePackageName = "$Config.PACKAGE_NAME.RESPONSE.$Config.CURRENT_GROUP_NAME"

    ClassName out = ClassName.get(tempResponsePackageName, responseName)
    ...
    //输出
    print2File(Config.FILE_PATH.RESPONSE, tempResponsePackageName, outTb.build())
...
//            println v.getClass()
            if (v instanceof Object) {
                ...
                print2File(Config.FILE_PATH.RESPONSE, tempResponsePackageName, javaBeanClass.build())

                classList.add(ClassName.get(tempResponsePackageName, responseName))
