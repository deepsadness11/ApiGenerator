...
//            println "p.type====$p.type"
    ...
    //创建
    print2File(Config.FILE_PATH.REQUEST_PATH, Config.PACKAGE_NAME.REQUEST_PARAM, paramType.build())
    //包名+文件名就是它的TypeName
    return ClassName.get(Config.PACKAGE_NAME.REQUEST_PARAM, paramName)
}
...
static void addPathAnnotation(String p, MethodSpec.Builder method) {
    ParameterSpec parameterSpec = ParameterSpec.builder(String.class, p)
            ...
            .addAnnotation(AnnotationSpec.builder(Path.class).addMember('value', p).build())
            .build();
...
//    if (!content) return ClassName.get(Config.PACKAGE_NAME.RESPONSE_PARAM, responseName)
...
//            println v.getClass()
            if (v instanceof Object) {
                ...
                print2File(Config.FILE_PATH.RESPONSE_PATH, Config.PACKAGE_NAME.RESPONSE_PARAM, javaBeanClass.build())

                classList.add(ClassName.get(Config.PACKAGE_NAME.RESPONSE_PARAM, responseName))
...
static addQueryParam(FieldParam p, MethodSpec.Builder mb) {
    ParameterSpec parameterSpec = ParameterSpec.builder(p.getClass(), p.field)
            .addModifiers(Modifier.FINAL)
            .addAnnotation(AnnotationSpec.builder(Query.class).addMember('value', p.field).build())
            .build();
    mb.addParameter(parameterSpec)
}
